...
public class WebSecurityConfiguration {
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.cors().and().csrf().disable().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS)
				...
				.hasAuthority("ROLE_ADMIN").requestMatchers("/api/v1/users/all").hasAnyAuthority("ROLE_ADMIN", "ROLE_EMPLOYEE")
				.requestMatchers("/api/v1/users/**").hasAnyAuthority("ROLE_ADMIN", "ROLE_USER", "ROLE_EMPLOYEE").requestMatchers("/api/v1/employees/**")
