...
export default function Redeem({ setShowRedeem }) {
    ...
    return (
        <div className="max-w-6xl mx-auto mt-8 rounded-lg overflow-hidden flex flex-col shadow-lg" style={{background: "#fff"}}>
            {/* Balance Header */}
            ...
            <div className="text-xl font-bold px-6 py-3 text-center bg-blue-50">
                Tổng tiền gốc và lãi: <span className="text-blue-600">{formatCurrency(totalAmount)}</span>
            </div>
                    ...
                    <table className="w-full text-left min-w-[900px]">
                        <thead>
                            <tr className="text-gray-500 border-b bg-gray-50">
                                ...
                                <th className="py-3 px-2 font-medium text-right">Lãi suất (%)</th>
                                <th className="py-3 px-2 font-medium text-right">Tiền lãi tới hiện tại</th>
                                <th className="py-3 px-2 font-medium text-center">Kỳ hạn (Tháng)</th>
                                <th className="py-3 px-2 font-medium text-center">Số ngày còn lại</th>
                        ...
                        <tbody>
                            {deposits.map((item) => (
                                <tr key={item.id} className="border-b last:border-b-0 hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="py-4 px-2 text-right font-mono">{formatCurrency(item.principal)}</td>
                                    <td className="py-4 px-2 text-right font-mono text-blue-600">{item.interestRate}%</td>
                                    <td className="py-4 px-2 text-right font-mono text-green-600">{formatCurrency(item.interest)}</td>
                                    <td className="py-4 px-2 text-center">{item.term}</td>
                                    <td className="py-4 px-2 text-center">
                                        <span className={`px-2 py-1 rounded-full text-sm ${
                                            item.daysLeft <= 0 ? 'bg-red-100 text-red-700' :
                                            item.daysLeft <= 5 ? 'bg-orange-100 text-orange-700' :
                                            item.daysLeft <= 15 ? 'bg-yellow-100 text-yellow-700' :
                                            'bg-green-100 text-green-700'
                                        }`}>
                                            {item.daysLeft <= 0 ? 'Đã đến hạn' : `${item.daysLeft} ngày`}
                                        </span>
                                    </td>
                                    <td className="py-4 px-2 text-center">
                                        <button 
